...
{
    /// <summary>
    /// Represents a destructible box in the game world that can take damage and drop items upon destruction.
    /// </summary>
    /// <remarks>The Box class inherits from RigidBody2D and manages its own health and destruction state. When the
    /// box's health reaches zero, it triggers an item drop based on predefined probabilities and removes itself from the
    /// scene. This class is intended for use in scenarios where interactive, destructible objects are needed, such as
    /// breakable containers or obstacles. Item drops are delegated to the first level, and the box is freed after
    /// attempting to drop an item.</remarks>
